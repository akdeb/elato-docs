...
### Choose edge server option (FREE USAGE)

<Tip>

**ELATO MODE (hosted):** Got your own ESP32 DIY hardware device? We offer a fully hosted server for free for up to 30 minutes per month. Register your device on the settings page and it will automatically connect to our edge server. Check out our Pricing page for more details.

</Tip>

<Note>

**DEV MODE (local):** Alternatively, you can run your own edge server locally by following the instructions below and in the Deno server README.

</Note>

**Pro Tip:** You can adjust this server setting in `firmware-arduino/Config.h`.

### Step 4 (Optional): Run the Deno edge server locally

From the repo root:

The commands below are the same on macOS, Linux, and Windows (via WSL2).
